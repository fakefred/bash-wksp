...
\begin{block}{Note}
    \verb|\.| matches a literal dot.

    Same goes for \verb|\(| \verb|\[| etc.
\end{block}
\end{frame}

\begin{frame}[fragile]
\frametitle{Regex patterns: \tt{[]}}
Brackets match one character inside, but \verb|^| and \verb|-| are special:
\begin{table}
    \centering
    \begin{tabular}{ll}
...
\begin{example}
    \verb|[A-C][01][0-9]| matches \tt{A00} up to \tt{C99}.
\end{example}
\end{frame}
...
\begin{example}
    \verb|\w+\s\w+| matches \tt{two words} and \tt{words with\_underscores}.
\end{example}

\begin{block}{Note}
    Things like \verb|[\w\s]| won't work.
\end{block}
\end{frame}
